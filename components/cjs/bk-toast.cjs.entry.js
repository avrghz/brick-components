...
const Icon = ({ variant }) => {
    ...
    return icon && index$1.h("iron-icon", { icon: icon, class: `bk-toast__icon bk-toast--${variant}` });
};
...
const Toast = class {
    ...
    render() {
        return (index$1.h(index$1.Host, { tabIndex: -1, "aria-hidden": "true", class: `bk-toast--${this.position}` }, this.toasts.map(({ message, id, title, variant = 'default', dismissible = true }) => (index$1.h("div", { key: id, id: id, class: "bk-toast" }, index$1.h(Icon, { variant: variant }), index$1.h("div", { class: `bk-toast__group bk-toast--${variant}` }, title && index$1.h("div", { class: "bk-toast__title" }, title), index$1.h("div", { class: "bk-toast__content" }, message), dismissible && (index$1.h("button", { class: "bk-button bk-button--text bk-toast__close-Btn", onClick: () => this.handleCloseButtonClick(id) }, index$1.h("iron-icon", { icon: "close", class: "bk-icon" })))))))));
    }
