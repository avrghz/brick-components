...
const ProgressLine = ({ progress, showPercentageText, textInside }) => {
    return [
        index.h("div", { class: "bk-progress-bar" },
            index.h("div", { class: "bk-progress-bar__outer" },
                index.h("div", { class: "bk-progress-bar__inner", style: {
                        width: `${progress}%`,
                    } },
                    index.h(ProgressText, { show: showPercentageText && textInside, isInnerText: true, progress: progress })))),
        index.h(ProgressText, { progress: progress, show: showPercentageText && !textInside }),
    ];
};

const Circle = (prop) => (index.h("path", { d: "\n        M 50 50\n        m 0 -47\n        a 47 47 0 1 1 0 94\n        a 47 47 0 1 1 0 -94\n        ", fill: "none", class: prop.cssName }));
const ProgressCircle = ({ progress, showPercentageText, strokeDash, strokeDashOffset, flip, }) => {
    return (index.h("div", { class: "bk-progress-circle" },
        index.h("style", null, `.bk-progress-circle__inner{
            ...
            `),
        index.h("svg", { viewBox: "0 0 100 100", class: flip ? 'bk-progress--flip' : '' },
            index.h(Circle, { cssName: "bk-progress-circle__outer" }),
            index.h(Circle, { cssName: "bk-progress-circle__inner" })),
        index.h(ProgressText, { show: showPercentageText, progress: progress })));
};
...
const ProgressBar = class {
    ...
    renderProgressBar(progressValue) {
        switch (this.barType) {
            case 'circle':
                return (index.h(ProgressCircle, { progress: progressValue, showPercentageText: this.showPercentageText, strokeDash: [295.31, 295.31], strokeDashOffset: 0 }));
            case 'dashboard':
                return (index.h(ProgressCircle, { progress: progressValue, showPercentageText: this.showPercentageText, strokeDash: [221.482, 295.31], strokeDashOffset: -36.9137, flip: true }));
            default:
                return (index.h(ProgressLine, { progress: progressValue, textInside: this.textInside, showPercentageText: this.showPercentageText }));
        }
    }
    render() {
        const progressValue = this.progress >= 0 ? (this.progress <= 100 ? this.progress : 100) : 0;
        return (index.h(index.Host, { role: "progressbar", "aria-valuenow": progressValue, "aria-valuemin": 0, "aria-valuemax": 100 }, index.h("div", { class: `bk-progress ${this.variant} ${this.textInside ? 'bk-progress--text-inside' : ''}` }, this.renderProgressBar(progressValue))));
    }
};
ProgressBar.style = indexCss;

exports.bk_progress_bar = ProgressBar;
